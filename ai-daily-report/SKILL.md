...
生成 **五类输出**：飞书 Newsrun 卡片、Markdown/飞书详细版文档、结构化沉淀文档、CSV 结构化数据、HTML 版日报；其中详细版固定配套三张趋势可视化图，并额外保留 Mck PPT 趋势后端的标准化输入。
...
5. 用户补充确认后，基于最终 Markdown 的三大趋势新建当日 `visual-spec-YYYY-MM-DD.json`，重新选择每张图的视觉主体、构图方向、标签和必要颜色，再渲染 Georgia O'Keeffe 可视化 HTML，截图并通过 16:9 / 不遮挡 QA。模板只复用视觉语法，不复用上一日报的图形、文案、标签、配色或 PNG。
...
### 输出二A：Georgia O'Keeffe 三趋势可视化

这是详细版日报的固定视觉层，不替代正文、CSV 或结构化沉淀。方法参考 [Georgia O'Keeffe Style](https://github.com/chengjialu8888/designer-consultancy/blob/main/ui-design/skills/georgia-okeeffe-style/SKILL.md)：选择一个具体对象或边界，近看、放大、删去噪音，让轮廓、开口、材质和必要颜色承担判断；不是复制具体画作，也不是把日报做成装饰海报。这里的“模板”只规定观看方法和排版约束，不是可复制的三张固定图。
...
- 每个趋势独立生成 1 张图，顺序与 Markdown、飞书详细版中的趋势 1/2/3 一致。
- 图中只使用已进入最终 Markdown 的标题、`🧭 批判性判断` 和极少量已核验数字/关键词；完整数据、新闻列表、来源链接留在飞书详细版正文。

#### 固定视觉规则

- 画布固定为 **16:9**；源图优先 `1440×810`，插入飞书文档时使用等比例 `960×540` 或同等 16:9 尺寸。
- 每张图固定包含：趋势编号/短标题、一个视觉主体、批判性判断；最多保留 3-4 个辅助标签或数字。
- 使用近看、局部裁切、有机轮廓、有限留白和“主色 + 支撑色 + 必要强调色”；避免模板化花朵、泛沙漠滤镜、渐变光球和信息图堆叠。
- 中文字优先保证可读性：标题和批判性判断字号先满足桌面截图，再检查移动端；不得让图中文字互相遮挡、溢出或被装饰形体穿过。
- 三张图共享同一视觉语法，但必须采用三种不同的构图架构和阅读路径，例如满幅叠字、竖向标本轨道、中心环绕或其他能由当日趋势证明合理的新结构；不能只在同一版式里替换文字、颜色或主体。
...
```bash
node AI_Blog/scripts/render-okeeffe-trends.mjs \
  --markdown reports/YYYY-MM-DD-ai-daily/ai-daily-YYYY-MM-DD.md \
  --visual-spec reports/YYYY-MM-DD-ai-daily/visual-spec-YYYY-MM-DD.json \
  --out reports/YYYY-MM-DD-ai-daily/ai-daily-YYYY-MM-DD-okeeffe.html

node AI_Blog/scripts/capture-okeeffe-trends.mjs \
  reports/YYYY-MM-DD-ai-daily/ai-daily-YYYY-MM-DD-okeeffe.html \
  reports/YYYY-MM-DD-ai-daily/trends-okeeffe/
```

截图目录必须包含 `trend-1-okeeffe.png`、`trend-2-okeeffe.png`、`trend-3-okeeffe.png` 和 `capture-metadata.json`。capture 脚本必须自动检查：3 张图、16:9 比例、桌面端无画布溢出、标题与判断文字无重叠、移动端无横向溢出；任一失败都不能写入飞书文档。
...
- 结构化条目必须保留链接、信号等级、事实性短摘要；只概括新闻发生了什么，不写三大趋势的批判性判断、风险推演、壁垒判断或“待验证”结论。不要使用旧的 `养虾实践` 字样。
