...
生成 **五份输出**：飞书 Newsrun 卡片、Markdown/飞书详细版文档、结构化沉淀文档、CSV 结构化数据、HTML 版日报。
...
5. 只有在完成用户补充环节后，才把最终版本写入飞书 Newsrun 结构化沉淀文档和飞书详细版文档。
6. 最后基于最终版结构化沉淀生成飞书 Newsrun 卡片并推送，同时保留本地 CSV/HTML 供长期运营复盘。
...
- 每个新闻 bullet 使用信号灯 emoji + 蓝色链接标题 + 一句简要亮点概括，和结构化沉淀文档里的短摘要/判断一致。
...
- 文档标题：`AI日报｜YYYY年M月D日`
- 顶部一级标题：`Newsrun沉淀`
- 顶部引用块：写当天 sharpen 后的趋势判断，并附信号等级说明。
- 日期层级：`YYYY年M月` → `M.D`
- 主体使用 3 列文档表格：`大类` / `板块` / `结构化条目`
- 大类固定使用 `偏fact类`、`偏观点类`，如有可执行动作则增加 `💪今天就做`
- 结构化条目必须保留链接、信号等级、短摘要；不要使用旧的 `养虾实践` 字样。
